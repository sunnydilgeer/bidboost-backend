#!/usr/bin/env python3
"""
Enhanced FTS Scraper - Handles multiple page formats with improved deadline extraction
Supports: GOV.UK Design System, EU Standard Forms, and Corrigendum notices
"""
import json
import time
import re
...
def detect_page_format(page):
    ...
    # Check for EU Standard Form (numbered sections like "two.1.1)")
    has_eu_sections = page.query_selector("text=/one\\.1\\)|two\\.1\\)|Section one/") is not None
    
    # Check for GOV.UK Design System
...
def parse_eu_corrigendum_deadline(full_text):
    ...
    # Look for corrected deadline in "Read" section after "Instead of"
    deadline_pattern = r'Instead of[^\n]*Date[^\n]*(\d{1,2}.*?202\d)[^\n]*Read[^\n]*Date[^\n]*(\d{1,2}.*?202\d)'
    match = re.search(deadline_pattern, full_text, re.DOTALL)
    
    if match:
        # Return the CORRECTED deadline (group 2), not the old one
        return match.group(2).strip()
    
    # Fallback: Look for "Time limit for receipt" in Section IV corrections
    fallback_patterns = [
...
def parse_eu_standard_format(page):
    ...
    # 🔥 NEW: Support both Roman numerals (II.1.1) and lowercase (two.1.1)
    # Title
    data["title"] = (
...
def parse_govuk_design_format(page):
    ...
    def get_dt_dd_text(label):
        """Get text from definition list (dt/dd pattern)"""
        try:
    ...
    if is_award:
        ...
        # Deadline - Try multiple labels (CRITICAL for bidding)
        data["deadline"] = (
...
def scrape_notice(page, url):
    """Scrape a single FTS notice - auto-detects format"""
    try:
        ...
        if page_format == "eu_standard":
            ...
            print(f"  [warning] Unknown page format, trying EU standard parser")
            data = parse_eu_standard_format(page)
        ...
        if not data.get("title"):
            print(f"  [warning] No title extracted")
        if not data.get("notice_type"):
            print(f"  [warning] Could not determine notice type")
...
def scrape_fts():
    """Scrape all open tenders from FTS"""
    data = []
    ...
    with sync_playwright() as p:
        try:
            ...
            print(f"[warning] Failed to launch headless chromium: {e}")
            print("[info] Trying with channel='chrome'...")
