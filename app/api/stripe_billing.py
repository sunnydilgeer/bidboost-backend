...
class CheckoutRequest(BaseModel):
    plan_type: str  # only "pro" is allowed from the client
...
async def create_checkout_session(
    ...
    IMPORTANT:
    - Starter is not purchasable (hidden / retention / legacy only)
    - Backend is the source of truth for price selection (founder pricing, etc.)
    """

    # ✅ SPEC: Block starter checkout
    if request.plan_type != "pro":
        raise HTTPException(status_code=400, detail="Only Pro upgrades allowed")

    # Get or create subscription record
    subscription = (
        db.query(FirmSubscription)
        .filter(FirmSubscription.firm_id == current_user.firm_id)
        .first()
    )
    ...
    # ✅ Founder pricing is selected server-side
    price_id, billing_price_label = select_price_id_for_plan(subscription, "pro")
...
async def stripe_webhook(request: Request, db: Session = Depends(get_db)):
    ...
    if event["type"] == "checkout.session.completed":
        ...
        if not firm_id:
            logger.error(f"No firm_id in session metadata: {session.get('id')}")
        ...
        if subscription_id:
            ...
            subscription = (
                db.query(FirmSubscription)
                .filter(FirmSubscription.firm_id == firm_id)
                .first()
            )
            ...
            if subscription:
                ...
                db.commit()
                logger.info(
                    f"✅ Upgraded {firm_id} to {plan} (billing: {billing_price}) via checkout.session.completed"
                )
            else:
        ...
        if subscription_id:
            subscription = (
                db.query(FirmSubscription)
                .filter(FirmSubscription.stripe_subscription_id == subscription_id)
                .first()
            )
            ...
            if subscription:
                ...
                db.commit()
                logger.info(
                    f"✅ Confirmed {subscription.plan} (billing: {subscription.billing_price}) for {subscription.firm_id} via invoice.paid"
                )
        ...
        subscription = (
            db.query(FirmSubscription)
            .filter(FirmSubscription.stripe_customer_id == customer_id)
            .first()
        )
        ...
        if subscription:
            ...
            db.commit()
            logger.info(
                f"✅ Updated {subscription.firm_id} to {plan} (billing: {billing_price}) via subscription.updated"
            )
        ...
        if subscription:
            ...
            # Revoke founder status on churn to prevent cancel-and-return exploit
            if subscription.billing_price == "pro_founder":
                subscription.founder_revoked_at = now
                logger.info(
                    f"⚠️ Revoked founder pricing for {subscription.firm_id} (churned)"
                )

            # ✅ SPEC: churn → starter (hidden)
            subscription.plan = "starter"
            subscription.billing_price = "starter"
            subscription.plan_expires_at = now
            db.commit()
            logger.info(
                f"✅ Downgraded {subscription.firm_id} to Starter via subscription.deleted"
            )
